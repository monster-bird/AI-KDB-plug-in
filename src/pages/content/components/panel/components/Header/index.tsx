...
function Header(): JSX.Element {
          ...
          <div className={tw`flex font-medium  h-full text-base relative`}>
            {items.map((item, index) => (
              <div
                ...
                className={tw`flex w-[32px] items-center justify-center cursor-pointer h-full 
              ${selectedItem === index ? "text-[#000000]" : "text-[#637381]"}
              ${item.key !== 0 ? "ml-[16px] " : ""}`}
                onClick={() => onTabChange(index)}
              >
                <span className={tw`text-[16px]`}>{item.label}</span>
              </div>
            ))}
            <span
              className={tw` h-[4px]  bg-[#3872e0] absolute`}
              style={{
    ...
    return (
      <Menu>
        <Menu.Item key={1}>
          <div
            onClick={() => {
              window.open("https://qm.qq.com/cgi-bin/qm/qr?k=cXBTRtayxZ2wDWVG4Fz2u1Y-IXga0wYl&jump_from=webapi&authKey=OjSGg37Ih6Qgy7YrqxwZ3DtBqvNXKJWsgEAbMBMnnpb0Zod8jbNC5xMLO5DLS39X");
            }}
          >
            <span>
              <QqOutlined />
            </span>
            <span className={tw`inline-flex ml-1`}>加入群组</span>
          </div>
        </Menu.Item>
        <Menu.Item key={2}>
          <div
  ...
  return (
    <div
      className={
        tw`w-full bg-white h-[44px] flex justify-between items-center px-[10px] box-border ` +
        ` m-header`
      }
    >
      <div
